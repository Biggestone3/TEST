...
from news import Article  # type: ignore
...
def crawler_1(myTimer: func.TimerRequest) -> None:
    ...
    def unify_date_mayadeen(date):
        ...
        if (
            date.split()[2] in month_dict
        ):  # the date in the mayadeen xml is as follows: Tue, 25 Mar 2025 09:35:09 GMT
            # and in datetime the args are as follows: year, month #, #day, #hour,
            #  minute, second, tzinfo
            hour = date.split()[4]
            date_to_return = datetime(
                int(date.split()[3]),
                month_dict[date.split()[2]],
                int(date.split()[1]),
                int(hour.split(":")[0]),
                int(hour.split(":")[1]),
                int(hour.split(":")[2]),
                tzinfo=UTC,
            )  # essentially we should should add GMT not UTC
            return date_to_return
        # in case we ended up with a false we'll leave the pydantic deal with it using
        # default ...
        return False
    ...
    def unify_dates(date):
        ...
        if date.split()[0] in month_dict:
            date = datetime(
                2025,
                month_dict[date.split()[0]],
                int(date.split()[1]),
                datetime.now().hour,
                0,
                0,
                tzinfo=UTC,
            )  # hours and minutes are not available
            return date
        day_month_year = date.split("|")[0].split(":")
        hour_minute = date.split("|")[1]
        date = datetime(
            int(day_month_year[0].split("-")[2]),
    ...
    def get_link_add_meta_data_to_dataset(main_url, news_tag):
        date = news_tag.find("div", class_="listingDate").contents[
            -1
        ]  # .contents will take the content of the direct children of this div as alist
        ...
        if scrape_link not in good_links:
            ...
            publish_date = str(
                date
            )  # there is the content and the language that are left
    ...
    async def scrape_main_page(main_link):
        ...
        for info in news:
            # if get_link_add_meta_data_to_dataset(main_link, info) == False:
            # #handling the case where the link is already visited
            #     return False
            (scrape_link, title_i, summary_i, url_i, publish_date_i) = (
                get_link_add_meta_data_to_dataset(main_link, info)
            )  #
            try:
                ...
                if (
                    content_i != ""
                ):  # checking if the content is null i.e. could be a vid photo ...
                    # print("appending to dataset")
                    dataset.append(
                        Article(
                            id=ObjectId(),
                            # it is gonna be 2 specific values one for annahar and
                            # another for mayadeen
                            source_id=UUID("f67322c8-069c-43a2-9097-6805d56dd436"),
                            title=title_i,
                            content=content_i,
                            summary=summary_i,
                            url=url_i,
            ...
            if url not in good_links:
                ...
                if content is not None:
                    paragraphs = content.find_all(
                        "p", recursive=False
                    )  # recursive = False will take only p tags of depth 1
                ...
                dataset.append(
                    Article(
                        id=ObjectId(),
                        source_id=UUID(
                            "8310fb18-4420-4cac-9330-9b178b8264dc"
                        ),  # 1 for mayadeen
                        title=title,
                        content=info,
                        summary="Something big happened!",
    ...
    # now the dataset has members of type Article so we need to initalize the beanie
    #  and start to add sstuff to the db
    async def init():
        try:
        ...
        # writing to db
        # with each function directly adding to the db as we go i.e. in the functions
        #  we will have an: await Article( ... ).insert()
        # and at the end we will call this init() function to start running it
        # using asyncio
        try:
